...
class MedicalRecordMapper
{
	...
	public function fetch($paginated=false, $filter = array(), $order=array())
	{
		...
		if(isset($filter['public_address'])){
			$where->equalTo("public_address", $filter['public_address']);
		}

		if(isset($filter['smart_contract_key'])){
			$where->equalTo("smart_contract_key", $filter['smart_contract_key']);
		}
