...
use Hospital\Model\HospitalEntity;

class HospitalMapper
{
	protected $tableName = 'hospital';
	protected $dbAdapter;
	protected $sql;
	...
	public function fetch($paginated=false, $filter = array(), $order=array(), $limit = null)
	{
		...
		if($paginated) {
		    $entityPrototype = new HospitalEntity();
	...
	public function save(HospitalEntity $hospital)
	{
		$hydrator = new ClassMethods();
		$data = $hydrator->extract($hospital);

		if ($hospital->getId()) {
			// update action
			$action = $this->sql->update();
			$action->set($data);
			$action->where(array('id' => $hospital->getId()));
		} else {
			// insert action
		...
		if (!$hospital->getId()) {
			$hospital->setId($result->getGeneratedValue());
		}
		return $result;
	}
	...
	public function getHospital($id)
	{
		...
		$hydrator->hydrate($result, $hospital);
